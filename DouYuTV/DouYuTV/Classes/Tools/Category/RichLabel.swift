...
class RichLabel: UILabel {
    ...
    override var attributedText: NSAttributedString? {
        didSet {
            ...
            let content  = [NSAttributedStringKey.foregroundColor:UIColor.lightGray]
            ...
            textStorage.addAttributes(content, range: NSRange(location: 0, length: attributedString.length))
            // 匹配用户名
            
            userRanges = ranges(from: "@.*?:")
            _ = userRanges.map { textStorage.addAttribute(.foregroundColor, value: UIColor.blueFontColor(), range: $0)
                textStorage.addAttributes(content, range:NSRange(location: 0, length: textStorage.length))
            }
            // 匹配话题
