...
class QQFlyingCarViewModel: BaseViewModel {
    private var count : Int = 0
    private var page : Int = 1
    private let timeStamp = Date().timeIntervalSince1970
}
extension QQFlyingCarViewModel{
    //pragma mark: -下拉刷新
    func requestData(type: HomeStyle,_ finishCallback : @escaping () -> ()){
        count = 0
        //0.定义参数
        let parameters = ["client_sys":"ios"]
        requestAnchorData(isGroupData: true, urlString: "https://apiv2.douyucdn.cn/gv2api/rkc/roomlistV1/\(type.qqCarID)/\(count)/20/ios", method: .get, parameters: parameters, finishedCallBack: finishCallback)
    }
    //pragma mark: -上拉加载
    ...
    func requestCyclePictureData(_ finishCallback : @escaping () -> ()){
        requestCyclePicture(urlString: "https://apiv2.douyucdn.cn/live/Slide/getSlideLists?cate_id=331&app_ver=\(app_ver)&client_sys=ios", method: .get, finishedCallBack: finishCallback)
    }
}
extension QQFlyingCarViewModel{
    func requestData(_ finishCallback : @escaping () -> ()){
        ...
        requestDynamicData(isGroupData: true, urlString: "http://mapi-yuba.douyu.com/wb/v3/catefeed/331", method: .get, parameters:parameters, headers: headers, finishedCallBack: finishCallback)
    }
