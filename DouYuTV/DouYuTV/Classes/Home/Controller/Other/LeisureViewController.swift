...
class LeisureViewController: BaseTotalViewController {
    //pragma mark: -进行懒加载(OnlineViewModel)
    private lazy var LeisureVM : LeisureViewModel = LeisureViewModel()
...
extension LeisureViewController{
    override func loadData() {
        print("下拉刷新成功")
        //pragma mark: -给父类中ViewModel进行赋值
        baseVM = LeisureVM
        LeisureVM.requestData { [weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
                self?.LeisureVM.classifyModel.removeAll()
            }
            self?.loadDataFinished()
        }
        LeisureVM.loadGameData {
            var onlineGames = self.LeisureVM.classifyModel
            onlineGames.removeSubrange(16..<onlineGames.count)
            self.topGameCollectionView.groups = self.LeisureVM.classifyModel
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = LeisureVM
        LeisureVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
