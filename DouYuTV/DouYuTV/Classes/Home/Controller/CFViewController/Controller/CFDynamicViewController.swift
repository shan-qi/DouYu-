...
class CFDynamicViewController: BaseTotalViewController {

    private lazy var cfDynamicVM : CFViewModel  = CFViewModel()
    /// 动态数据
    private lazy var dynamics : [DynamicModel] = [DynamicModel]()
}
extension CFDynamicViewController{
...
extension CFDynamicViewController{
    override func loadData() {
        ...
        baseVM = cfDynamicVM
        cfDynamicVM.requestData { [weak self] in
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = cfDynamicVM
        cfDynamicVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
...
extension CFDynamicViewController : UICollectionViewDelegateFlowLayout {
