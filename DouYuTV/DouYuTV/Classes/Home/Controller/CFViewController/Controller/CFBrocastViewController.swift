...
class CFBrocastViewController: UIViewController {
    
    private lazy var CFDownTitileView : PageSubTitleView = {
    ...
    private lazy var pageContentView : PageContentView = {
        ...
        //2.确定子控制器
        var childVcs = [CFSubViewController]()
        for type in homeTypes {
            let anchorVC = CFSubViewController()
            anchorVC.home = type
    ...
    private func loadTypesData() ->[HomeStyle] {
        let path = Bundle.main.path(forResource: "CF", ofType: "plist")
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.addSubview(CFDownTitileView)
        view.addSubview(pageContentView)
        view.insertSubview(pageContentView, belowSubview: CFDownTitileView)
    }
}
//遵守pageTitleViewDelegate协议
extension CFBrocastViewController : pageSubTitleViewDelegate{
