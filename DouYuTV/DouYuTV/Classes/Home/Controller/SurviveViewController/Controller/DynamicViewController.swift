...
class DynamicViewController: BaseTotalViewController {
    
    private lazy var surviveDynamicVM : SurviveViewModel  = SurviveViewModel()
    /// 动态数据
...
extension DynamicViewController{
    override func loadData() {
        ...
        baseVM = surviveDynamicVM
        surviveDynamicVM.requestData { [weak self] in
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = surviveDynamicVM
        surviveDynamicVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
