//
//  DOTA2DynamicViewController.swift
//  DouYuTV
//
...
class DOTA2DynamicViewController: BaseTotalViewController {

    private lazy var dota2DynamicVM : DOTA2ViewModel  = DOTA2ViewModel()
    /// 动态数据
    private lazy var dynamics : [DynamicModel] = [DynamicModel]()
}
extension DOTA2DynamicViewController{
...
extension DOTA2DynamicViewController{
    override func loadData() {
        ...
        baseVM = dota2DynamicVM
        dota2DynamicVM.requestData { [weak self] in
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = dota2DynamicVM
        dota2DynamicVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
...
extension DOTA2DynamicViewController : UICollectionViewDelegateFlowLayout {
