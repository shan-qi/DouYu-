...
class DOTA2SubViewController: BaseViewController {
    var home : HomeStyle!
    fileprivate lazy var DOTA2VM : DOTA2ViewModel = DOTA2ViewModel()
...
extension DOTA2SubViewController{
    fileprivate func loadData() {
        DOTA2VM.requestData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
    fileprivate func loadMoreData() {
        DOTA2VM.requestMoreData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
...
extension DOTA2SubViewController : UICollectionViewDataSource{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return DOTA2VM.anchorGroups.count
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return DOTA2VM.anchorGroups[section].anchors.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if home.dotaID == "2_3" {
            ...
            quidCell = cell
            cell.anchor = DOTA2VM.anchorGroups[indexPath.section].anchors[indexPath.item]
        }else{
...
extension DOTA2SubViewController : xqLOLGameViewDelegate{
...
extension DOTA2SubViewController : UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        //获取主播信息
        let anchor = DOTA2VM.anchorGroups[indexPath.section].anchors[indexPath.item]
