//
//  KingDynamicViewController.swift
//  DouYuTV
//
...
class KingDynamicViewController: BaseTotalViewController {
    private lazy var kingDynamicVM : KingViewModel  = KingViewModel()
    /// 动态数据
    private lazy var dynamics : [DynamicModel] = [DynamicModel]()

}
extension KingDynamicViewController{
...
extension KingDynamicViewController{
    override func loadData() {
        ...
        baseVM = kingDynamicVM
        kingDynamicVM.requestData { [weak self] in
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = kingDynamicVM
        kingDynamicVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
...
extension KingDynamicViewController : UICollectionViewDelegateFlowLayout {
