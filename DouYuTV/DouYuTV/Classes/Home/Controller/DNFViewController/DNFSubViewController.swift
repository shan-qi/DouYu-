...
class DNFSubViewController: BaseViewController {
    var home : HomeStyle!
    fileprivate lazy var DNFVM : DNFViewModel = DNFViewModel()
...
extension DNFSubViewController{
    fileprivate func loadData() {
        DNFVM.requestData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
        DNFVM.requestLivingAnchorData {
            if self.DNFVM.livingAnchorModels.isEmpty{
                self.anchorView.isHidden = true
                self.anchorView.height=0
                self.collectionView.y=0
            }else{
                if self.home.dnfID == "2_40" {
                    self.anchorView.isHidden = false
                    self.anchorView.y = 0
                    self.anchorView.cfs = self.DNFVM.livingAnchorModels
                }else{
                    self.anchorView.isHidden = true
                    self.collectionView.y = 0
                }
    ...
    fileprivate func loadMoreData() {
        DNFVM.requestMoreData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
...
extension DNFSubViewController : UICollectionViewDataSource{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return DNFVM.anchorGroups.count
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return DNFVM.anchorGroups[section].anchors.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if home.dnfID == "2_40" {
            ...
            quidCell = cell
            cell.anchor = DNFVM.anchorGroups[indexPath.section].anchors[indexPath.item]
        }else{
...
extension DNFSubViewController : xqLOLGameViewDelegate{
...
extension DNFSubViewController : UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        //获取主播信息
        let anchor = DNFVM.anchorGroups[indexPath.section].anchors[indexPath.item]
