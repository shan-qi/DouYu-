//
//  StimulateDynamicViewController.swift
//  DouYuTV
//
...
import UIKit
class StimulateDynamicViewController: BaseTotalViewController {
    private lazy var stimulateDynamicVM : StimulateViewModel  = StimulateViewModel()
    /// 动态数据
    private lazy var dynamics : [DynamicModel] = [DynamicModel]()
}
extension StimulateDynamicViewController{
...
extension StimulateDynamicViewController{
    override func loadData() {
        ...
        baseVM = stimulateDynamicVM
        stimulateDynamicVM.requestData { [weak self] in
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = stimulateDynamicVM
        stimulateDynamicVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
...
extension StimulateDynamicViewController : UICollectionViewDelegateFlowLayout {
