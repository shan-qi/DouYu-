//
//  HostGameSubViewController.swift
//  DouYuTV
//
...
class HostGameSubViewController: BaseViewController {
    var home : HomeStyle!
    fileprivate lazy var HostGameVM : HostGameViewModel = HostGameViewModel()
...
extension HostGameSubViewController{
    fileprivate func loadData() {
        HostGameVM.requestData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
    fileprivate func loadMoreData() {
        HostGameVM.requestMoreData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
...
extension HostGameSubViewController : UICollectionViewDataSource{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return HostGameVM.anchorGroups.count
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return HostGameVM.anchorGroups[section].anchors.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        var quidCell = UICollectionViewCell()
        if home.hostGameID == "2_19" {
            ...
            quidCell = cell
            cell.anchor = HostGameVM.anchorGroups[indexPath.section].anchors[indexPath.item]
        }else{
...
extension HostGameSubViewController : xqLOLGameViewDelegate{
...
extension HostGameSubViewController : UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        //获取主播信息
        let anchor = HostGameVM.anchorGroups[indexPath.section].anchors[indexPath.item]
