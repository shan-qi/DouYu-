//
//  WholeArmySubViewController.swift
//  DouYuTV
//
...
class WholeArmySubViewController: BaseViewController {
    var recordDistance : Double = 0.0

    var home : HomeStyle!
    fileprivate lazy var WholeArmyVM : WholeArmyViewModel = WholeArmyViewModel()
...
extension WholeArmySubViewController{
    fileprivate func loadData() {
        WholeArmyVM.requestData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
    fileprivate func loadMoreData() {
        WholeArmyVM.requestMoreData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
...
extension WholeArmySubViewController : UICollectionViewDataSource{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return WholeArmyVM.anchorGroups.count
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return WholeArmyVM.anchorGroups[section].anchors.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        var quidCell = UICollectionViewCell()
        if home.wholeArmyID == "2_349" {
            ...
            quidCell = cell
            cell.anchor = WholeArmyVM.anchorGroups[indexPath.section].anchors[indexPath.item]
        }else{
...
extension WholeArmySubViewController : xqLOLGameViewDelegate{
...
extension WholeArmySubViewController : UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        //获取主播信息
        let anchor = WholeArmyVM.anchorGroups[indexPath.section].anchors[indexPath.item]
