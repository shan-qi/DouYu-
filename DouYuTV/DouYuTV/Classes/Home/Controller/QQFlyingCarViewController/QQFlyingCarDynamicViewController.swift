//
//  QQFlyingCarDynamicViewController.swift
//  DouYuTV
//
...
class QQFlyingCarDynamicViewController: BaseTotalViewController {
    private lazy var qqflyingCarDynamicVM : QQFlyingCarViewModel  = QQFlyingCarViewModel()
    /// 动态数据
    private lazy var dynamics : [DynamicModel] = [DynamicModel]()
}
extension QQFlyingCarDynamicViewController{
...
extension QQFlyingCarDynamicViewController{
    override func loadData() {
        ...
        baseVM = qqflyingCarDynamicVM
        qqflyingCarDynamicVM.requestData { [weak self] in
    ...
    override func loadMoreData() {
        print("上拉加载成功")
        baseVM = qqflyingCarDynamicVM
        qqflyingCarDynamicVM.loadData {[weak self] in
            DispatchQueue.main.async {
                self?.collectionView.reloadData()
            }
            self?.loadDataFinished()
...
extension QQFlyingCarDynamicViewController : UICollectionViewDelegateFlowLayout {
