//
//  QQFlyingCarViewController.swift
//  DouYuTV
//
...
//pragma mark: -QQ飞车
class QQFlyingCarViewController: BaseTotalViewController {
    private lazy var QQFlyingCarVM : QQFlyingCarViewModel = QQFlyingCarViewModel()
    //pragma mark: -轮播数据
    ...
    private lazy var QQFlyingCarTitleView : PageSubTitleView = {
        let style = XQTitleStyle()
        style.isScrollEnable = false
        style.isShowBottomLine = true
        style.font = UIFont.systemFont(ofSize: 17.0)
        let titles = ["直播","精彩视频","专区动态"]
    ...
    private lazy var pageContentView : PageContentView = {
        ...
        childVcs.append(QQFlyingCarBrocastViewController())
        childVcs.append(QQFlyingCarVideoViewController())
        childVcs.append(QQFlyingCarDynamicViewController())        
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(QQFlyingCarTitleView)
        view.addSubview(pageContentView)
        view.insertSubview(pageContentView, belowSubview: QQFlyingCarTitleView)
    }
    deinit {
        cycleView.delegate = nil
        QQFlyingCarTitleView.delegate = nil
    }
}
extension QQFlyingCarViewController{
    override func loadData() {
        baseVM = QQFlyingCarVM
        QQFlyingCarVM.requestCyclePictureData {[weak self] in
            if (self?.QQFlyingCarVM.cycleModels.isEmpty)!{
                self?.cycleView.isHidden = true
                self?.QQFlyingCarTitleView.y -= xqCycleH
                self?.pageContentView.y -= xqCycleH
            }else{
              self?.cycleView.cycleModels = self?.QQFlyingCarVM.cycleModels
            }
        }
    }
}
extension QQFlyingCarViewController : xqGeneralCycleViewDelegate{
...
extension QQFlyingCarViewController : pageSubTitleViewDelegate{
