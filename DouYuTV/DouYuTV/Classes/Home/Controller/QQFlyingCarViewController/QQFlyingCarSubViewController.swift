//
//  QQFlyingCarSubViewController.swift
//  DouYuTV
//
...
class QQFlyingCarSubViewController: BaseViewController {
    var home : HomeStyle!
    fileprivate lazy var QQFlyingCarVM : QQFlyingCarViewModel = QQFlyingCarViewModel()
...
extension QQFlyingCarSubViewController{
    fileprivate func loadData() {
        QQFlyingCarVM.requestData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
    fileprivate func loadMoreData() {
        QQFlyingCarVM.requestMoreData(type: home!) {[weak self] in
            self?.collectionView.reloadData()
            self?.loadDataFinished()
        }
    }
...
extension QQFlyingCarSubViewController : UICollectionViewDataSource{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return QQFlyingCarVM.anchorGroups.count
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return QQFlyingCarVM.anchorGroups[section].anchors.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        var quidCell = UICollectionViewCell()
        if home.qqCarID == "2_331" {
            ...
            quidCell = cell
            cell.anchor = QQFlyingCarVM.anchorGroups[indexPath.section].anchors[indexPath.item]
        }else{
...
extension QQFlyingCarSubViewController : xqLOLGameViewDelegate{
...
extension QQFlyingCarSubViewController : UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        //获取主播信息
        let anchor = QQFlyingCarVM.anchorGroups[indexPath.section].anchors[indexPath.item]
